function useCountdown() {
  ...
  function init(cd, tz) {
    ...
    _initClock(deadline, tz);
  }

  function _initClock(deadline, tz) {
    // ---# DOM SETUP
    _setupDOM();

    // ---# UPDATE THE CLOCK
    setInterval(() => _updateClock(deadline, tz), 1000);
  }

  function _updateClock(deadline, tz) {
    const t = _getTimeRemaining(deadline, tz);
  ...
  function _getTimeRemaining(deadline, timeZone = "America/New_York") {
    const dateNow = Date.parse(
      String(new Date().toLocaleString("en-US", { timeZone }))
    );
